...
function QuickAction({ icon, label, onPress, accent = 'teal' }) {
  const accentStyle = {
    teal: styles.quickGlyphTeal,
    navy: styles.quickGlyphNavy,
    amber: styles.quickGlyphAmber,
  }[accent] || styles.quickGlyphTeal;

  return (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.quickAction, pressed && styles.quickActionPressed]}>
      <View style={[styles.quickGlyph, accentStyle]}>
        {icon}
      </View>
...
function EntityCard({ children }) {
  return (
    <View style={styles.entityCard}>
      <View style={styles.entityAccent} />
      {children}
    </View>
  );
}
...
export default function OfficeDashboardScreen({ navigation }) {
  ...
  const quickActions = [
    {
      key: 'refresh',
      icon: <Ionicons name="refresh" size={18} color={palette.teal600} />,
      label: 'Refresh',
    ...
    {
      key: 'history',
      icon: <Ionicons name="reader-outline" size={18} color={isDark ? palette.ink900 : palette.navy700} />,
      ...
      <View style={styles.sectionStack}>
        <Card style={styles.panelCard}>
          ...
          {dashboard.recentReadings.length ? (
            <View style={styles.list}>
              {dashboard.recentReadings.map((item) => (
                <EntityCard key={item.id}>
                  <View style={styles.entityHeader}>
                    <View style={styles.rowCopy}>
            ...
            <MessageBanner tone="info">No readings have been submitted yet.</MessageBanner>
          )}
        </Card>
      ...
      <Card style={styles.quickActionsCard}>
        <View style={styles.quickActionsTopRow}>
          <View style={styles.quickActionsHeader}>
            ...
            <Text style={styles.quickActionsBody}>Quick tools</Text>
          </View>
      ...
      <Card style={styles.navigationCard}>
        <View style={styles.navigationHeader}>
          ...
          <Text style={styles.navigationMeta}>
            {isAdmin ? `${sections.length} views` : 'Readings view'}
          </Text>
        </View>
